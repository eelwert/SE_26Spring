...
functions: list[PluginFunction] = [
    ...
    # Member A: Templates & Textures & Furniture
    PluginFunction(name="apply_scene_template", title="场景模板", category="layout",
                   description="应用预设场景模板(0=滨水,1=商业,2=枢纽)", enabled=True, risk="low",
                   schemaSummary="template_id", averageMs=500),
    PluginFunction(name="apply_road_texture", title="道路纹理", category="asset",
                   description="切换道路纹理材质", enabled=True, risk="low",
                   schemaSummary="texture_id", averageMs=400),
    PluginFunction(name="apply_pavement_texture", title="人行道纹理", category="asset",
                   description="切换人行道铺装纹理", enabled=True, risk="low",
                   schemaSummary="texture_id", averageMs=400),
    PluginFunction(name="delete_furniture", title="删除新增城市家具", category="asset", description="删除之前新增到CG网格上的指定类型3D家具", enabled=True, risk="low", schemaSummary="asset_id", averageMs=300),
    PluginFunction(name="place_furniture", title="放置城市家具", category="asset",
                   description="在城市对象周围放置3D家具（野餐椅/小消防罐/小黄鸭）", enabled=True, risk="low",
                   schemaSummary="asset_id, count, spacing", averageMs=600),
    ...
    PluginFunction(name="generate_terrain", title="生成山丘地形", category="environment",
                   ...
                   schemaSummary="hill_height, noise_scale", averageMs=800),
    PluginFunction(name="generate_lake", title="生成湖泊", category="environment",
                   description="生成圆形湖泊水面", enabled=True, risk="low",
                   schemaSummary="lake_size, ripple_strength", averageMs=600),
    PluginFunction(name="generate_river", title="生成河流", category="environment",
                   description="沿曲线生成河流水面", enabled=True, risk="low",
                   schemaSummary="river_width, seed", averageMs=500),
    PluginFunction(name="add_boat", title="添加船只", category="environment",
                   description="在河流上添加动态船只", enabled=True, risk="low",
                   schemaSummary="boat_scale, flow_speed", averageMs=400),
    # Member D: Simulation + Layout
    PluginFunction(name="start_simulation", title="启动交通仿真", category="simulation",
                   description="启动车辆+行人+红绿灯仿真", enabled=True, risk="medium",
                   schemaSummary="car_density, pedestrian_density", averageMs=1800),
    PluginFunction(name="stop_simulation", title="停止仿真", category="simulation",
                   description="停止仿真并清除动态元素", enabled=True, risk="low",
                   schemaSummary="none", averageMs=300),
    PluginFunction(name="apply_layout", title="坐标布局生成", category="layout",
                   description="根据坐标点集生成道路布局", enabled=True, risk="medium",
                   schemaSummary="points", averageMs=1000),
