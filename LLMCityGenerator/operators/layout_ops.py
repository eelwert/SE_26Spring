...
class CG_OT_PreviewPointLayout(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        self.report({"INFO"}, f"Preview: {len(obj.data.vertices)} verts, {len(obj.data.edges)} edges")
        return {"FINISHED"}

    @staticmethod
    def _parse_coords(text):
        """Parse "x,y;x,y;..." or "x,y,z;x,y,z;..." into points + connections."""
        if not text:
            return None, None
        points = []
        ...
        if len(points) < 2:
            return None, None
        return points, None  # sequential connection
...
class CG_OT_ApplyPointLayout(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        obj.select_set(True)
        self.report({"INFO"}, f"RoadLayout: {len(obj.data.vertices)} verts, {len(obj.data.edges)} edges")
        return {"FINISHED"}
